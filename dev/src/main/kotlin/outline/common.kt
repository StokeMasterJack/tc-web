...
private fun saveOutline(workshopKey: String, text: String) {
    val ff = outlineFile(workshopKey)
    println("Saving outline to [$ff]...")
    ff.writeText(text)
    println("  Saving outline complete!")
}
...
public fun loginToCheckvistSync(): String {
    ...
    val (_, _, text) = Fuel.get(url).responseString()
...
//    val foo = Fuel.get(url).responseString()
    val (_, _, text) = Fuel.get(url).responseString()
